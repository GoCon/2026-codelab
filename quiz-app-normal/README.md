...
- ランダムに選ばれた 5 問を出題
- ヘッダー右上の地球儀メニューから日本語 / English を切り替え可能
- 問題文・選択肢・解説・タイトルも選択言語に合わせて切り替え
- `quizes.yaml` の `mode` が `normal` / `both` の問題だけを出題
...
2. `quiz-app-normal/gas/telemetry.gs` の内容を貼り付ける
3. **デプロイ > 新しいデプロイ**
4. 種類を **ウェブアプリ** にする
5. 実行ユーザーを **自分**
6. アクセスできるユーザーを **全員**
...
- `PerfectScores` シートには normal の全問正解時に送信されたニックネーム、完走タイム、正解数、回答完了日時、`mode`（常に `normal`）、`attempt_id` が保存されます

## GitHub Pages デプロイについて

GitHub Pages 向けの静的成果物は `make build-pages` で生成できます。  
`.github/workflows/quiz-app-normal-github-pages.yml` では **build job と artifact upload だけ動かし、deploy job は公開日まで skip** しています。  
リポジトリの GitHub Pages は公開日まで有効化しない前提のため、workflow は `actions/configure-pages` を呼ばずに静的成果物の build 確認だけを行います。

workflow では次の Variables を使います。

| Variable 名 | 必須 | 説明 |
|---|---|---|
| `QUIZ_APP_NORMAL_TELEMETRY_ENDPOINT` | 任意 | Apps Script の Web アプリ URL（未設定時は記録送信を無効化） |
| `QUIZ_APP_NORMAL_PREVIEW_UNLOCK_CODE` | 任意 | フッター 10 タップ後に開く hidden Keyword ポップアップで使うコード（未設定時は `gofar,gotogether`） |

- `QUIZ_APP_NORMAL_TELEMETRY_ENDPOINT` が空でも静的サイト自体はビルドでき、既定では記録送信は無効のままです
- `QUIZ_APP_NORMAL_PREVIEW_UNLOCK_CODE` を未設定のまま使う場合、既定の hidden Keyword は `gofar,gotogether` です
...
`mode` は省略可能です。値は次の 3 つです。`quiz-app-normal` では `extra` の問題は配信対象から除外されます。

- `both`（既定値）: `quiz-app-normal` でも出題候補に入る
- `normal`: `quiz-app-normal` で出題する
- `extra`: 共有データには残せるが、`quiz-app-normal` では出題しない
