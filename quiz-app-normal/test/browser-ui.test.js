...
test('language switch localizes the app UI and active quiz content', () => {
  ...
  assert.equal(app.localStorage.get('quiz-app-normal-language'), 'en');
...
test('normal mode score keeps extra-mode actions hidden', () => {
  const app = createHarness();
  ...
  assert.equal(app.elements.get('score-card').style.display, 'block');
  assert.equal(app.elements.get('challenge-btn').style.display, 'none');
  assert.equal(app.elements.get('retry-extra-mode-btn').style.display, 'none');
  assert.equal(app.elements.get('score-home-btn').style.display, 'none');
});

test('normal app HTML no longer includes extra mode assets or overlay', () => {
  assert.doesNotMatch(appHTML, /extra-mode\.css/);
  assert.doesNotMatch(appHTML, /extra-mode\.js/);
  assert.doesNotMatch(appHTML, /id="extra-mode-overlay"/);
  assert.doesNotMatch(appHTML, /id="extra-mode-clock"/);
  assert.match(extraModeScript, /createExtraModeController/);
  assert.match(extraModeCSS, /\.extra-mode-screen \{/);
});
