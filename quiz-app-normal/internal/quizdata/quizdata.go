...
func LoadFromBase(basePath string) ([]quizhandler.Quiz, map[string]string, error) {
	...
	quizzes = FilterNormalModeQuizzes(quizzes)
...
// FilterNormalModeQuizzes keeps quizzes that are available to the normal-mode app.
func FilterNormalModeQuizzes(quizzes []quizhandler.Quiz) []quizhandler.Quiz {
	filtered := make([]quizhandler.Quiz, 0, len(quizzes))
	for _, quiz := range quizzes {
		if quiz.Mode == quizhandler.QuizModeExtra {
			continue
		}
